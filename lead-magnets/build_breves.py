...
# · el problema en pocas líneas · lo esencial · el principio operativo · el
# mismo CTA a la llamada de evaluación. Todo el contenido sale de la guía
# larga; acá no se inventa nada nuevo.
#
# Las reglas de voz están en specs/voz.md (publicadas en Notion como "Cómo
# escribimos"). Léelas antes de tocar cualquier texto de acá.
...
EXTRA = """
  ...
  .mini .mh { font-size: 9.5pt; color: #3D6058; display: block; margin-top: 0.8mm; }
  /* El principio operativo cierra las cuatro guías, justo antes del CTA. */
  .brief .principio { font-family: 'Lora', serif; font-style: italic; font-size: 12.5pt;
                      color: #0E3A2F; margin: 5mm 0 0; padding-left: 6mm;
                      border-left: 2.5px solid #E65F2B; line-height: 1.35; }
...
def mini(titulo, desc, cita=None, hecho=None):
    q = f'<span class="mq">{cita}</span>' if cita else ""
    # "Cómo se ve hecho" es parte del entregable cuando la guía pide actuar.
    h = f'<span class="mh"><strong>Hecho:</strong> {hecho}</span>' if hecho else ""
    return f'<div class="mini"><div class="mt">{titulo}</div><div class="md">{desc}</div>{q}{h}</div>'
...
"""
  + mini("1 · Vida con Propósito", "Dirección, identidad y estructura del tiempo. El trabajo te entrega un rol, metas y horarios, y con eso un sentido de valor propio y de eficacia. Al jubilar eso desaparece, y puede vivirse como una pérdida o trabajarse como una reinvención: la diferencia la hace el tiempo con que te prepares.",
  ...
  + mini("5 · Finanzas con Propósito", "Cuánto sabes de lo que tienes, y para qué lo guardas. Un monto sin destino no da tranquilidad, porque no hay con qué compararlo: primero se define la vida que quieres y después se calcula cuánto cuesta.",
  ...
  """    <div class="kicker">El efecto dominó</div>
    ...
    <div class="rule"></div>
    <p>Los cinco están conectados: cuando uno se debilita, empuja hacia abajo a los demás. A eso le llamamos el efecto dominó, y por eso conviene comenzar por el pilar más débil: una acción ahí rinde más que cinco repartidas.</p>
"""
  + step("→", "<strong>Propósito</strong> afecta a Finanzas y a Social: sin dirección ningún monto se siente suficiente, y el círculo que venía con el rol se va con él.")
  + step("→", "<strong>Físico</strong> afecta a Social y a Finanzas: sin autonomía cuesta sostener los vínculos, y el gasto en cuidados es el que más se subestima.")
  + step("→", "<strong>Mental y Cognitivo</strong> afecta a Físico y a Propósito: sin descanso no hay energía, y sin claridad mental ningún plan se sostiene.")
  + step("→", "<strong>Social</strong> afecta a Mental y a Físico: la soledad crónica se comporta como un factor de riesgo para la salud.")
  ...
  + """
    ...
    <p>Vuelve a las cinco alertas de la página anterior y marca la que más se parece a tu semana. Ese es el pilar por el que te conviene comenzar.</p>
    <div class="note">
      <p><strong>Ojo:</strong> esto es orientación personal, no un diagnóstico, y no reemplaza a tu médico ni a tu asesor financiero.</p>
    </div>
    <div class="principio">¿Vas a actuar por inercia o con intención?</div>""",
  "Si quieres que revisemos tus cinco pilares juntos,", "pdf-pilares-breve")

# -------------------------------------------------------------------- PLAN --
guia('plan-breve',
  "Cuatro acciones que puedes<br>dejar hechas antes de jubilar",
  "Se hacen una sola vez y quedan hechas. Ninguna te pide cambiar tu rutina ni sostener un hábito nuevo.",
  """    <div class="kicker">El problema</div>
    ...
    <p>Lo que suele ocurrir es que esos asuntos no llegan solos ni con tiempo. El dinero, la salud, los cuidados y dónde vas a vivir aparecen juntos, muchas veces empujados por algo que se rompió, y hay que resolverlos rápido y bajo presión.</p>
    <div class="hl">Decidir con tiempo y reaccionar bajo presión llevan a resultados distintos.</div>
    <p>Estas cuatro no te piden decidir: te piden hacerlas. Cada una se ejecuta una vez y queda lista, y abajo dice cómo se ve cuando lo está.</p>
    <h3>Las dos primeras</h3>
"""
  + mini("1 · Un número", "Cuánto cuesta un mes de tu vida: lo que cuesta sostener tu vida normal durante treinta días, con número. Es la base de cualquier otro cálculo, y las estimaciones suelen quedar bastante por debajo del gasto real.",
         ...
         "tienes el número escrito en alguna parte, con fecha, y sabes qué incluye.")
  + mini("2 · Una hora médica", "Ese examen o control que vienes postergando. Uno, no todos. Postergarlo solo lo mueve a un momento en que vas a tener menos margen para decidir con calma.",
         ...
         "ya te lo hiciste y sabes qué dice el resultado. Agendarlo todavía no es hacerlo."),
  """    <div class="kicker">Las otras dos</div>
    <h2>Una persona<br>y una fecha</h2>
    <div class="rule"></div>
"""
  + mini("3 · Una persona", "Quién va a decidir por ti si en algún momento no puedes hacerlo. Si no lo defines tú, lo define la circunstancia, y suele quedar en quien esté más cerca ese día, sin saber qué querías. Es la más barata de las cuatro y la que más peso le saca a tu familia.",
  ...
  + """
    ...
    <p>Si intentas las cuatro esta semana, es probable que no hagas ninguna. Elige la que te dio más incomodidad al leerla, que suele ser la que más falta hace, y ejecútala hasta que quede hecha. Las otras tres van a seguir estando la semana que viene.</p>
    <div class="note">
...
"""
  + step("1", "<strong>Puso el horizonte por escrito.</strong> Dejó de rondarle en la cabeza y pasó a ser una dirección.")
  ...
  + step("4", "<strong>Instaló hábitos alineados con ese horizonte.</strong> Los que servían a su objetivo, en vez de una lista genérica.")
  ...
  + """
    ...
    <p>Empezar, mucho más que sostenerlo. El dinero, porque un patrimonio en el papel no es lo mismo que uno disponible cuando se necesita. Y las conversaciones con su familia, que lo apoyó y aun así costaron.</p>
